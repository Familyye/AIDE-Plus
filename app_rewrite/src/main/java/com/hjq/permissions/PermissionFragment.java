...
public final class PermissionFragment extends Fragment implements Runnable {
	 ...
	 * 开启权限申请
	 */
	public static void launch(Activity activity, List<String> permissions, OnPermissionInterceptor interceptor,
	...
	public void requestSpecialPermission() {
		...
		for (int i = allPermissions.size() - 1; i >= 0; i--) {
			...
			StartActivityManager.startActivityForResult(this,
					PermissionApi.getSmartPermissionIntent(activity, PermissionUtils.asArrayList(permission)),
	 ...
	 */
	public void splitTwiceRequestPermission(final Activity activity, final List<String> allPermissions,
			List<String> firstPermissions, final int requestCode) {

		final ArrayList<String> secondPermissions = new ArrayList<>(allPermissions);
		for (String permission : firstPermissions) {
			secondPermissions.remove(permission);
		}

		PermissionFragment.launch(activity, firstPermissions, new OnPermissionInterceptor() {
			...
			public void onGranted(List<String> permissions, boolean allGranted) {
				...
				PermissionUtils.postDelayed(new Runnable() {
					...
					@Override
					public void run() {
						PermissionFragment.launch(activity, secondPermissions, new OnPermissionInterceptor() {
						}, new OnPermissionCallback() {
							...
							public void onDenied(List<String> permissions, boolean doNotAskAgain) {
								...
								for (int i = 0; i < allPermissions.size(); i++) {
									grantResults[i] = PermissionUtils.containsPermission(secondPermissions,
	...
	public void onRequestPermissionsResult(int requestCode, String[] permissions, int[] grantResults) {
		...
		for (int i = 0; i < permissions.length; i++) {
			grantResults[i] = PermissionApi.recheckPermissionResult(activity, permissions[i],
